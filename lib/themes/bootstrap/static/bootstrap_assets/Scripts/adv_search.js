...
j(document).ready(function () {
	...
	j('.ep_multi_input').closest('form').addClass('form-inline row');	
	j('.ep_multi_input').addClass('col-md-10');	
	j('.ep_multi_help').addClass('col-md-1');
	j('table.ep_search_fields th').addClass('col-md-2');
	j('table.ep_search_fields').addClass('table');
	j('.ep_multi_input select, .ep_multi_input input').addClass('form-control');	

	//remove the old styling elements and allow the bs table to do it instead
	j('.ep_multi_input').removeClass('ep_multi_input');	
	j('.ep_multi_heading').removeClass('ep_multi_heading');	
	j('.ep_multi_help').removeClass('ep_multi_help');	
